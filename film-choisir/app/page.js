...
export default function Home() {
  ...
  async function handleMarkSeen(movie) {
        ...
        try {
                ...
                // Si l'enregistrement echoue, le film reste affiche comme "vu" a
          // l'ecran mais pourrait reapparaitre dans une recherche future.
        }
  }
...
{!movie.imdbRating && !movie.rottenTomatoes && (
                ...
                <button
                  ...
                  disabled={seenIds.has(movie.id)}
                  onClick={() => handleMarkSeen(movie)}
                >
{seenIds.has(movie.id) ? "Vu ✓" : "Marquer comme vu"}
</button>
  </div>
  </div>
          ))}
